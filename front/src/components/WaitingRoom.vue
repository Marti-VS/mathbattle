...
export default {
  ...
  computed: {
    ...
    partidas() {
      ...
      if (getSocket().partidas) {
        ...
        this.filterWins(getSocket().partidas);
      } else {
        this.playing = false;
      }
  ...
  mounted() {
    ...
    subSocket((state) => {
      console.log(state.sala);
      if (state.sala == false || state.sala == null) {
        setTimeout(() => {
          window.location.href = "/join";
        }, 3000);
      } else {
        if (state.owner == this.myId) {
          this.owner = true;
        } else {
          if (state.owner_id == getState().usuari.id) {
            this.owner = false;
            this.kick = true;
            setTimeout(() => {
              window.location.href = "/join";
              ...
              <div
                ...
                <div>
                  ...
                  <div class="mt-3 text-center sm:mt-5">
                    ...
                    <div class="mt-2">
                      <p class="text-sm text-gray-500">Uneix-te!</p>
                    </div>
                  </div>
